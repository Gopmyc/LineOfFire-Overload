...
function View:OnUpdate(nDeltaTime)
	...
	if bHasMovementInput then
		...
		nBodyYawDelta				= bHasYawFollowTarget and self:ComputeSmoothedBodyYawDelta(nBodyYawDeltaTarget, nDeltaTime) or 0.0
		nUpdatedFreelookYaw			= self:Clamp(nFreelookYawClamped - nBodyYawDelta, -self.MAX_FREELOOK_YAW_ANGLE, self.MAX_FREELOOK_YAW_ANGLE)
	end
...
function View:IsMovementInputPressed()
	local oController	= self._private.oController

	if not oController then return false end

	local nLeftKey			= oController.LEFT_KEY
	local nAltLeftKey		= oController.ALT_LEFT_KEY
	local nRightKey			= oController.RIGHT_KEY
	local nForwardKey		= oController.FORWARD_KEY
	local nAltForwardKey	= oController.ALT_FORWARD_KEY
	local nBackwardKey		= oController.BACKWARD_KEY
	local bMoveLeft			= (nLeftKey and Inputs.GetKey(nLeftKey) or false) or (nAltLeftKey and Inputs.GetKey(nAltLeftKey) or false)
	local bMoveRight		= nRightKey and Inputs.GetKey(nRightKey) or false
	local bMoveForward		= (nForwardKey and Inputs.GetKey(nForwardKey) or false) or (nAltForwardKey and Inputs.GetKey(nAltForwardKey) or false)
	local bMoveBackward		= nBackwardKey and Inputs.GetKey(nBackwardKey) or false
